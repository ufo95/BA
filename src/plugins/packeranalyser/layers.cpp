...
void add_to_layer(drakvuf_t drakvuf, packeranalyser *p, uint64_t page_gfn, int layer_index){
	...
	if(!layer){//New layer to be added to list
		layer = (layer_entry *)g_malloc0(sizeof(layer_entry));
		p->layers = g_list_append(p->layers, layer);
	}
...
void add_to_layer_with_address(drakvuf_t drakvuf, vmi_instance_t vmi, packeranalyser *p, uint64_t from_va, uint64_t page_gfn){
	...
	} else if ((to_layer-1)!=p->current_exec_layer){//This page is already in a layer and its not the one currently allowed for writing update the written_from var. If so that means multi frame packer
		//printf("multiframe\n");
...
void add_traps_to_old_layer(frame *tmp, packeranalyser *p, drakvuf_t drakvuf){
...
void switch_to_layer_with_address(drakvuf_t drakvuf, vmi_instance_t vmi, packeranalyser *p, uint64_t pa){
	...
	layer_entry *to_entry = (layer_entry *)g_list_nth_data(p->layers, to_layer);
	if(!to_entry){
		printf("%s: no to entry found", __FUNCTION__);
		throw -1;
	}

	GSList *to_layer_iterator = (GSList *)to_entry->frames;
	...
	if(from_layer_iterator==NULL){
		printf("No from_layer_iterator found \n");
		throw -1;
	}

	while(from_layer_iterator!=NULL){
		add_traps_to_old_layer((frame *)from_layer_iterator->data, p, drakvuf);
		from_layer_iterator = from_layer_iterator->next;
	}

	p->number_of_transition++;
	dump_layer_to_folder(p, vmi, to_layer);
